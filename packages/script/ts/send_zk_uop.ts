...
const PAYMASTER_SIGNER_PRIVATE_KEY = process.env
  .PAYMASTER_SIGNER_PRIVATE_KEY as `0x${string}`;
...
async function main() {
  ...
  const salt = `0x${BigInt("00").toString(16)}` as Hex;
  ...
  const buffer = Buffer.from(userOperationHash, "hex");
  ...
  const { proof, publicSignals } = await groth16.fullProve(
    INPUT,
    join(__dirname, "/../../packages/circom-circuits/build/main_js/main.wasm"),
    join(__dirname, "/../../packages/circom-circuits/setup/circuit_final.zkey")
  );
...
// Helper functions
function createInitCode(identityCommitment: Hex, salt: Hex): Hex {
...
async function sponsorUserOperation(
  ...
  userOperation.paymasterAndData = concat([
    ...
    await paymasterAccount.signMessage({ message: hash }),
  ]);
