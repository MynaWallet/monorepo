...
use halo2_rsa::{
    AssignedBigUint, AssignedRSAPubE, AssignedRSAPublicKey, AssignedRSASignature, BigUintConfig, BigUintInstructions,
    Fresh, RSAConfig, RSAInstructions, RSAPubE, RSAPublicKey, RSASignature,
};
...
fn slice_biguint(
    ...
    // assign witnesses
    for (i, src_limb) in src.value().iter_u64_digits().enumerate() {
        let do_src_and_dest_overlap =
            since < (i + 1) * biguint_chip.limb_bits() && until > i * biguint_chip.limb_bits();
        if !do_src_and_dest_overlap {
            continue;
        }

        let beginning_part_max_width = until - i * biguint_chip.limb_bits();
        let beginning_part_width = beginning_part_width.min(beginning_part_max_width);
        let beginning_part = BigUint::from(src_limb) % (BigUint::one() << beginning_part_width);
        parts.push(ctx.load_witness(Fr::from(beginning_part.to_u64().unwrap())));

        let ending_part_max_width = until - i * biguint_chip.limb_bits() - beginning_part_width;
        let ending_part_width = ending_part_width.min(ending_part_max_width);
        let ending_part = (BigUint::from(src_limb) >> beginning_part_width) % (BigUint::one() << ending_part_width);
        parts.push(ctx.load_witness(Fr::from(ending_part.to_u64().unwrap())));

        if until < (i + 1) * biguint_chip.limb_bits() {
            let final_part = src_limb >> final_part_offset;
            parts.push(ctx.load_witness(Fr::from(final_part)));
        }
    }
