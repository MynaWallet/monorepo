...
impl State {
    ...
    fn update(self, action: Action) -> State {
        ...
        if new_state.byte_kind == ByteKind::Type {
        ...
        } else if new_state.byte_kind == ByteKind::Length && new_state.parsed_bytes == new_state.header_size {
            new_state.byte_kind = ByteKind::Payload;
        } else if new_state.byte_kind == ByteKind::Payload
        ...
        if let Action::Parse(byte) = action {
            if new_state.byte_kind == ByteKind::Length {
